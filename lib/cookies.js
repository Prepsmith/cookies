...
function Cookies(request, response, options) {
  ...
  if (options) {
    if (Array.isArray(options)) {
      ...
      this.keys = new Keygrip(options, 'sha1', 'hex')
      ...
      this.keys = Array.isArray(options.keys) ? new Keygrip(options.keys, 'sha1', 'hex') : options.keys
...
Cookies.prototype.get = function(name, opts) {
  var header, match, value, raw_value, signature
  ...
  raw_value = decodeURIComponent(value.split('--')[0])
  signature = value.split('--')[1]
  if (this.keys.sign(raw_value) != signature) return
...
Cookies.prototype.set = function(name, value, opts) {
  ...
  if (opts && signed) {
    ...
    cookie.value = encodeURIComponent(cookie.value) + '--' + this.keys.sign(cookie.toString())
  }
